...
// ──── Payment orders (shared with the main app) ──────────────────────────
...
export async function deletePaymentByOrderId(orderId) {
  if (hasMongo()) {
    try {
      const db = await getMongoDb();
      await db.collection("payments").deleteOne({ order_id: orderId });
      return true;
    } catch (e) { console.error("deletePaymentByOrderId mongo error:", e?.message); }
  }
  const orders = readJSON(PAYMENTS_FILE);
  const filtered = orders.filter((o) => o.order_id !== orderId);
  if (filtered.length === orders.length) return false;
  writeJSON(PAYMENTS_FILE, filtered);
  return true;
}

// ──── Live presence (read + cleanup only; written by the main app) ────────
